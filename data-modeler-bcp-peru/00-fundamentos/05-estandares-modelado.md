...
> ### Este estándar es exigible, y se comprueba
>
> Un estándar que nadie verifica se convierte en decoración, y entonces enseña justo lo contrario
> de lo que pretende: que las convenciones son opcionales.
>
> `validacion/estandares.sql` contrasta los 10 esquemas contra este documento —tipos monetarios,
> ubigeo, documento, booleanos, nomenclatura, claves, índices operativos, trazabilidad— y corre
> como parte de `validar.sh`. **12 reglas, todas en `OK`.**
>
> Donde el laboratorio se desvía **a propósito**, la excepción está declarada en ese script con su
> motivo. Las tres que hay hoy:
>
> | Desviación | Por qué |
> |---|---|
> | Solo el **caso 01** lleva las 4 columnas de auditoría | Repetirlas en 121 tablas ahogaría el DDL que el alumno tiene que leer. Se implementan donde se enseñan. **En producción no es aceptable** |
> | `GENERATED BY DEFAULT` en vez de `ALWAYS` | Las cargas deterministas insertan identificadores explícitos para que las cifras sean reproducibles, y el caso 05 necesita la clave `-1` del miembro desconocido |
> | `fecha_operacion` declarada `TIMESTAMP` | Reproduce el nombre que tiene en el sistema fuente que simula. El estándar aplica a las columnas que el modelo **introduce**, no a las que hereda |
>
> Una desviación documentada es una decisión. Una silenciosa es una deuda.
