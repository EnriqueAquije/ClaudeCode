...
| **Frontera** | **Parcial** | Los enunciados están llenos de reglas con `>` y `>=`. Hay pruebas de frontera en el caso 04 (partición) y el 06 (vigencias), no sistemáticas |
...
**Si quieres llevar el laboratorio más lejos**, el mejor siguiente paso son las pruebas de frontera
sistemáticas: los enunciados están llenos de reglas con `>` y `>=`, y casi ninguna prueba toca el
punto exacto. Un monto **igual** al umbral, una vigencia que empieza y termina el mismo día, el
último día del mes en el borde de una partición, el 28 de febrero.
...
```bash
./validacion/validar.sh          # las cinco redes, sobre los 10 casos
./validacion/concurrencia.sh     # la sexta: dos sesiones a la vez (aparte, necesita 2 conexiones)
```

Ejecuta, por cada caso: modelo físico → datos → consultas → **reglas de calidad** → **pruebas
negativas**; y después, sobre el conjunto, el **cumplimiento del estándar de modelado** y las
**cifras documentadas**. Cinco redes distintas, que atrapan cosas distintas.
