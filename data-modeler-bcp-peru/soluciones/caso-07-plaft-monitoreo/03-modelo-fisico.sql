...
-- Relación 1:N: un caso agrupa VARIAS alertas, y cada alerta pertenece como mucho a UN caso.
-- No es N:M, y la diferencia importa: si una misma alerta pudiera sustentar dos casos harian
-- falta una tabla puente y decidir a cual de los dos se le imputa. Aqui se eligio 1:N porque
-- el analista consolida: cuando dos alertas resultan ser el mismo hecho, se unen en un caso.
...
--    DEBER DE RESERVA: el acceso se restringe a nivel de fila con RLS + FORCE.
--
--    SOBRE EL REGISTRO DE ACCESOS: la tabla bitacora_acceso_ros modela COMO se registraria,
--    pero NO se alimenta sola. Un SELECT no dispara un trigger, asi que en PostgreSQL esto
--    exige pgaudit, o exponer el ROS unicamente a traves de una funcion SECURITY DEFINER que
--    escriba la bitacora antes de devolver la fila. Las filas que trae el laboratorio estan
--    sembradas a mano. Se deja asi a proposito: implementarlo de verdad excede el caso, y
--    fingir que funciona enseñaria a confiar en un control que no existe.
-- =====================================================================================
...
-- FORCE es imprescindible, y es el error mas frecuente al implementar RLS:
-- sin el, el DUENO de la tabla se salta las politicas. En un banco el dueno del esquema
-- suele ser el usuario con el que corre el proceso batch, asi que "ENABLE" a secas deja
-- el ROS legible justo para la cuenta que mas consultas ejecuta.
ALTER TABLE ros FORCE ROW LEVEL SECURITY;
